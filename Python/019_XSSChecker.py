...
# --- グローバル変数として定義 ---
# XSSチェック用のペイロード
payload = "<h1>XSS_TEST</h1>"

# --- 脆弱性チェック関数を定義 ---
def check_xss_vulnerability(session, attack_url):
    """指定されたURLに対してCross-Site Scripting(XSS)の脆弱性チェックを行う"""
    ...
    try:
        ...
        if payload in response.text:
            print(f"    [!!!] Found potential XSS vulnerability! The payload was reflected in the response.")
            return True
        else:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="A simple XSS checker for DVWA.")
    ...
    for param_to_test, value_list in query_params.items():
        ...
        if check_xss_vulnerability(session, attack_url):
            overall_vulnerable = True

    print("\n" + "="*40)
    if overall_vulnerable:
        print("[!!!] SUMMARY: The target URL appears to be vulnerable to Cross-Site Scripting (XSS).")
    else:
        print("[+] SUMMARY: No XSS vulnerabilities were found.")
    print("="*40)
